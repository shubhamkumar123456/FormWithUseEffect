...
const Login = (props) => {
  ...
  const emailChangeHandler = (event) => {
    ...
    setFormIsValid(
      event.target.value.includes('@') && enteredCollegeName.trim().length >2 && passwordState.isValid
    );
  };
  ...
  const passwordChangeHandler = (event) => {
    ...
    setFormIsValid(
    emailState.isValid && enteredCollegeName.trim().length >2 && event.target.value.trim().length > 6 
    )
  };
